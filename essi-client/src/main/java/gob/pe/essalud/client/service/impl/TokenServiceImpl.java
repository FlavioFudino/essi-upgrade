...
public class TokenServiceImpl extends BaseService implements TokenService {
    ...
    public Map token(String autorization) {
        this.loggerDebug("Inicio token", formatterHour.format(new Date()));
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin token", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map retoken(Map token) {
        this.loggerDebug("Inicio retoken", formatterHour.format(new Date()));
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", Constantes.URL_TOKEN_USER_SECURITY);
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin retoken", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map tokenRegistro(TokenRegistroRequestDto token) {
        this.loggerDebug("Inicio tokenRegistro", formatterHour.format(new Date()));
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin tokenRegistro", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map tokenActivar(TokenRegistroRequestDto token) {
        this.loggerDebug("Inicio tokenActivar", formatterHour.format(new Date()));
        String url = UriComponentsBuilder.fromUriString(this.getProperty(Constantes.URL_ENDPOINT_BASE_TRX) + "token-registro/")
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin tokenActivar", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map validarToken(TokenRegistroRequestDto token) {
        this.loggerDebug("Inicio validarToken", formatterHour.format(new Date()));
        String url = UriComponentsBuilder.fromUriString(this.getProperty(Constantes.URL_ENDPOINT_BASE_TRX) + "token-registro/")
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin validarToken", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map existeTokenActivo(TokenRegistroRequestDto token) {
        this.loggerDebug("Inicio existeTokenActivo", formatterHour.format(new Date()));
        String url = UriComponentsBuilder.fromUriString(this.getProperty(Constantes.URL_ENDPOINT_BASE_TRX) + "token-registro/")
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin existeTokenActivo", formatterHour.format(new Date()));
        return response.getBody();
    }

    @Override
    public Map sendToken(Map paramInput) {
        this.loggerDebug("Inicio sendToken", formatterHour.format(new Date()));
        ...
        ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, httpEntity,
                Map.class);
        this.loggerDebug("Fin sendToken", formatterHour.format(new Date()));
        return response.getBody();
    }
}
